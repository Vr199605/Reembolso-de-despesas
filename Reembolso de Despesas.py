...
URL_APP = "https://reembolso-de-despesas.streamlit.app/" # Link do seu app
...
def enviar_aviso_ao_gabriel(solicitacao):
    ...
    # Adicionado link para acessar as coisas pendentes no corpo do e-mail
    corpo = f"""
    Olá Gabriel Coelho,
...
with aba_guia:
    ...
    **Nenhuma despesa é aprovada sem comprovante.** Você pode selecionar múltiplos arquivos de uma vez.
    ...
    # Texto solicitado antes do botão de download
...
with aba_colab:
    ...
    for i, item in enumerate(st.session_state.items_reembolso):
        with st.container():
            ...
            # Formatação visual dos limites (estilo alerta)
